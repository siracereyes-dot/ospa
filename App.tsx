...
// IMPORTANT: Paste your NEW Deployment URL from "Deploy > New Deployment" here
...
const App: React.FC = () => {
  ...
  const handleSave = async () => {
    if (!isFormValid) {
      ...
      setActiveTab('basic');
      alert("Missing Profile Data: Division, School, Name, and PDF are required.");
      return;
    }
    ...
    try {
      ...
      alert(`Success! Data for ${data.candidateName} was sent to Google Sheet & Drive.`);
      setShowValidationErrors(false);
    } catch (error) {
      console.error(error);
      alert("An error occurred. Make sure your Apps Script is deployed for 'Anyone' and the URL is correct.");
    } finally {
    ...
    <div className="flex justify-between items-center mb-10">
      <div className="flex items-center gap-5">
        ...
        <div>
          ...
          <p className="text-[10px] text-slate-400 font-bold uppercase tracking-widest mt-1">Section Overview</p>
        </div>
      </div>
      <div className="text-right">
        <span className="text-[9px] font-bold text-slate-400 uppercase tracking-widest block mb-1">Raw Points</span>
      ...
      <div className="elegant-card p-10 rounded-[2.5rem] mb-8">
        <div className="flex justify-between items-center mb-8">
          ...
          <span className="bg-indigo-50 text-indigo-600 px-5 py-2 rounded-2xl text-[10px] font-black uppercase tracking-widest">
            Weighted: {getVal().toFixed(2)}
          </span>
        </div>
        <div className="grid grid-cols-1 md:grid-cols-4 gap-4 mb-8 bg-slate-50 p-8 rounded-3xl border border-slate-100">
          ...
          <div className="flex items-end">
            <button 
              ...
              className="w-full bg-slate-900 text-white p-4 rounded-2xl hover:bg-black transition-all font-black text-[10px] uppercase tracking-widest shadow-lg shadow-slate-200"
            >
              Add Award
            </button>
          </div>
          ...
          <table className="w-full text-left">
            ...
            <tbody className="divide-y divide-slate-50">
              ...
              {(data[category] as Achievement[]).length === 0 && (
                <tr><td colSpan={4} className="py-10 text-center text-slate-400 italic text-xs">No entries added.</td></tr>
              )}
            </tbody>
          </table>
        </div>
  ...
  const renderServiceSection = (title: string, category: keyof OSPAScoreState, levels: Level[]) => {
    ...
    return (
      <div className="elegant-card p-10 rounded-[2.5rem] mb-8">
        ...
        <div className="grid grid-cols-1 md:grid-cols-3 gap-4 mb-8 bg-slate-50 p-8 rounded-3xl border border-slate-100">
          ...
          <div className="flex items-end">
            <button 
              ...
              className="w-full bg-slate-900 text-white p-4 rounded-2xl hover:bg-black transition-all font-black text-[10px] uppercase tracking-widest shadow-xl shadow-slate-200"
            >
              Add Entry
            </button>
          </div>
          ...
          <table className="w-full text-left">
            ...
            <tbody className="divide-y divide-slate-50">
              ...
              {(data[category] as ServiceEntry[]).length === 0 && (
                <tr><td colSpan={2} className="py-10 text-center text-slate-400 italic text-xs">No entries added.</td></tr>
              )}
            </tbody>
          </table>
        </div>
  ...
  return (
    <div className="min-h-screen pb-20 bg-slate-50/50">
      <header className="bg-white/80 backdrop-blur-2xl border-b border-slate-100 sticky top-0 z-[100] h-24 shadow-sm">
        <div className="max-w-7xl mx-auto px-6 h-full flex items-center justify-between">
          <div className="flex items-center gap-5">
            <div className="w-12 h-12 bg-indigo-600 rounded-[1.25rem] flex items-center justify-center text-white text-xl shadow-xl shadow-indigo-200">
              <i className="fas fa-pen-nib"></i>
            </div>
            <div>
              <h1 className="text-2xl font-black text-slate-900 tracking-tight leading-none uppercase">OSPA <span className="text-indigo-600">Scorer</span></h1>
              <p className="text-[10px] text-slate-400 font-bold tracking-[0.2em] uppercase mt-1">Professional Evaluation Suite</p>
            </div>
          </div>
          <div className="hidden md:flex items-center gap-10">
            <div className="text-right">
              <p className="text-[10px] font-bold text-slate-400 uppercase tracking-widest mb-1">Final Computation</p>
              <p className="text-4xl font-black text-indigo-600 tabular-nums tracking-tighter">{totals.grandTotal.toFixed(2)}</p>
            </div>
            <button 
              onClick={handleSave}
              disabled={isSubmitting}
              className={`px-14 py-4.5 rounded-2xl font-black text-xs uppercase tracking-widest transition-all flex items-center gap-3 ${isSubmitting ? 'bg-slate-100 text-slate-300' : 'bg-slate-900 text-white hover:bg-black hover:scale-[1.02] shadow-2xl shadow-slate-200'}`}
            >
      ...
      <main className="max-w-7xl mx-auto px-6 mt-12 grid grid-cols-1 lg:grid-cols-12 gap-12">
        <aside className="lg:col-span-3">
          <div className="bg-white rounded-[3rem] p-10 border border-slate-100 sticky top-32 space-y-12 shadow-sm">
            <div className="space-y-8">
              <div className="flex items-center gap-3">
                <div className="w-2.5 h-8 bg-indigo-600 rounded-full"></div>
                <h2 className="text-xs font-black text-slate-800 uppercase tracking-widest">Candidate Profile</h2>
              </div>
              
              <div className="space-y-6">
                <div>
                  <label className="text-[9px] font-bold text-slate-400 uppercase block mb-2.5 tracking-widest ml-1">Division</label>
                  <select 
                ...
                <div>
                  <label className="text-[9px] font-bold text-slate-400 uppercase block mb-2.5 tracking-widest ml-1">School Name</label>
                  <input 
                ...
                <div>
                  <label className="text-[9px] font-bold text-slate-400 uppercase block mb-2.5 tracking-widest ml-1">Nominee Name</label>
                  <input 
                ...
                <div>
                  <label className="text-[9px] font-bold text-slate-400 uppercase block mb-3.5 tracking-widest ml-1">Portfolio (PDF)</label>
                  <div className={`relative border-2 border-dashed rounded-3xl p-8 text-center group cursor-pointer transition-all ${data.movFile ? 'border-emerald-200 bg-emerald-50' : (showValidationErrors && !data.movFile ? 'border-red-200 bg-red-50 animate-pulse' : 'border-slate-100 hover:border-indigo-300 bg-slate-50')}`}>
                    <input type="file" accept=".pdf" className="absolute inset-0 w-full h-full opacity-0 cursor-pointer" onChange={handleFileUpload} />
                    <i className={`fas ${data.movFile ? 'fa-check-circle text-emerald-500' : 'fa-file-pdf text-slate-300'} text-4xl mb-4`}></i>
            ...
            <nav className="space-y-3 pt-10 border-t border-slate-50">
              {[
                { id: 'basic', label: 'Dashboard', icon: 'fa-house-chimney' },
                { id: 'contests', label: 'Journalism', icon: 'fa-trophy' },
                { id: 'leadership', label: 'Leadership', icon: 'fa-user-tie' },
                { id: 'services', label: 'Extensions', icon: 'fa-people-group' },
                { id: 'interview', label: 'Interview', icon: 'fa-microphone-lines' }
              ].map(tab => (
                <button
                  key={tab.id}
                  onClick={() => setActiveTab(tab.id)}
                  className={`w-full flex items-center gap-5 px-6 py-4.5 rounded-[1.5rem] text-left text-[11px] font-black uppercase tracking-widest transition-all ${activeTab === tab.id ? 'bg-indigo-600 text-white shadow-xl shadow-indigo-100' : 'text-slate-500 hover:bg-slate-50 hover:text-indigo-600'}`}
                >
        ...
        <section className="lg:col-span-9 space-y-12 pb-20">
          {activeTab === 'basic' && (
            <div className="space-y-12 animate-in fade-in duration-700">
              <div className="elegant-card p-12 rounded-[3.5rem]">
                <div className="flex items-center gap-6 mb-12">
                  <div className="w-16 h-16 bg-indigo-50 text-indigo-600 rounded-[2rem] flex items-center justify-center text-3xl">
                    <i className="fas fa-chart-line"></i>
                  </div>
                  <div>
                    <h2 className="text-3xl font-black text-slate-900 tracking-tight">IPCRF Performance</h2>
                    <p className="text-slate-400 text-sm font-medium tracking-wide">Historical Ratings (Last 5 Years)</p>
                  </div>
                </div>
                
                <div className="grid grid-cols-1 md:grid-cols-5 gap-5">
                  {data.performanceRatings.map((entry) => (
                    <div key={entry.year} className="p-8 bg-slate-50 rounded-[2.5rem] border border-slate-100 group transition-all hover:border-indigo-300">
                      <label className="text-[10px] font-black text-slate-400 uppercase tracking-widest mb-4 block">{entry.year}</label>
                      <input 
                ...
                <div className="mt-12 p-12 bg-slate-900 rounded-[3rem] flex items-center justify-between text-white shadow-3xl shadow-slate-200">
                  <div className="flex items-center gap-12">
                    ...
                    <div>
                      <p className="text-[11px] font-bold text-slate-400 uppercase tracking-widest mb-3">5-Year Aggregate Average</p>
                      <p className="text-7xl font-black tracking-tighter tabular-nums">{averageRating.toFixed(3)}</p>
                    </div>
                  </div>
                  <div className="px-12 py-5 bg-indigo-600 text-white rounded-3xl text-xs font-black uppercase tracking-widest shadow-xl">
              ...
              <div className="grid grid-cols-2 md:grid-cols-4 gap-8">
                {[
                  { label: 'Journalism', value: (totals.indiv + totals.group + totals.special + totals.pub).toFixed(2), icon: 'fa-pen-clip', color: 'bg-amber-500' },
                  { label: 'Leadership', value: totals.leadershipTotal.toFixed(2), icon: 'fa-shield-halved', color: 'bg-indigo-500' },
                  { label: 'Extensions', value: (totals.extension + totals.innovations + totals.speakership + totals.books + totals.articles).toFixed(2), icon: 'fa-hands-clapping', color: 'bg-emerald-500' },
                  { label: 'Interview', value: totals.interviewTotal.toFixed(2), icon: 'fa-comments', color: 'bg-blue-500' }
                ].map((stat, i) => (
                  <div key={i} className="elegant-card p-10 rounded-[3rem] transition-all hover:-translate-y-3 group bg-white">
                    <div className={`w-16 h-16 ${stat.color} bg-opacity-10 ${stat.color.replace('bg-', 'text-')} rounded-2xl flex items-center justify-center mb-10 text-3xl group-hover:scale-110 transition-all`}>
          ...
          {activeTab === 'leadership' && (
            <div className="elegant-card p-12 rounded-[3.5rem] animate-in fade-in">
              {renderSectionHeader('Leadership & Service', 'fa-award', totals.leadershipTotal.toFixed(2), 'bg-indigo-500')}
              <div className="grid grid-cols-1 md:grid-cols-4 gap-5 mb-12 p-10 bg-slate-50 rounded-[3rem] border border-slate-100">
                 <div className="space-y-2">
                   <label className="text-[10px] font-black text-slate-400 uppercase ml-2 tracking-widest">Level</label>
                ...
                <div className="space-y-2">
                   <label className="text-[10px] font-black text-slate-400 uppercase ml-2 tracking-widest">Rank/Position</label>
                ...
                <div className="md:col-span-2 flex items-end">
                   <button 
                     ...
                     className="w-full py-5.5 bg-slate-900 text-white rounded-2xl font-black text-[11px] uppercase tracking-widest hover:bg-black transition-all shadow-xl shadow-slate-200"
                   >
                     Record Position
                   </button>
                </div>
              ...
              <div className="space-y-5">
                {data.leadership.map(item => (
                  <div key={item.id} className="flex justify-between items-center p-10 bg-white rounded-[2.5rem] border border-slate-100 hover:border-indigo-300 transition-all group">
                    <div className="flex items-center gap-10">
                      ...
                      <div>
                        ...
                        <p className="text-[11px] text-indigo-500 font-black uppercase tracking-widest mt-1.5">{item.level} Officer</p>
                      </div>
                    </div>
          ...
          {activeTab === 'services' && (
            <div className="space-y-6 animate-in fade-in">
               {renderServiceSection('5. Extension Services (Org/Facilitator)', 'extensionServices', [Level.NATIONAL, Level.REGIONAL, Level.DIVISION])}
               {renderServiceSection('Innovations & Advocacy Projects', 'innovations', [Level.NATIONAL, Level.REGIONAL, Level.DIVISION, Level.DISTRICT, Level.SCHOOL])}
          ...
          {activeTab === 'interview' && (
            <div className="elegant-card p-12 rounded-[3.5rem] animate-in fade-in">
              {renderSectionHeader('Interview Performance', 'fa-microphone-lines', totals.interviewTotal.toFixed(2), 'bg-blue-500')}
              <div className="space-y-24 max-w-4xl mx-auto py-12">
                {[
                  { key: 'principles', label: 'Journalism Principles & Media Ethics' },
                  { key: 'leadership', label: 'Mentorship & Leadership Capacity' },
                  { key: 'engagement', label: 'Community & School Participation' },
                  { key: 'commitment', label: 'Commitment to the Profession' },
                  { key: 'communication', label: 'Communication Competence' }
                ].map(indicator => (
                  <div key={indicator.key} className="space-y-10">
                    <div className="flex justify-between items-end">
                      ...
                      <div className="px-7 py-3.5 bg-blue-50 text-blue-600 rounded-[1.75rem] text-sm font-black tabular-nums tracking-widest shadow-sm">
                    ...
                    <input 
                      ...
                      className="w-full h-4 bg-slate-100 rounded-full appearance-none cursor-pointer accent-blue-600 transition-all"
                    />
                  </div>
                ))}
              </div>
